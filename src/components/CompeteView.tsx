...
import { Trophy, Target, Users, Zap, TrendingUp, Calendar, ChevronRight, Award, Flame } from "lucide-react"
    ...
    <div className="space-y-8 pb-12">
      ...
      <Tabs defaultValue="challenges" className="w-full" onValueChange={setActiveSubTab}>
        ...
        <AnimatePresence mode="wait">
          <TabsContent value="challenges" className="space-y-6 pt-6">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex -space-x-2">
                          {challenge.participants.map((p, idx) => (
                            <div key={idx} className="w-6 h-6 rounded-full bg-white/10 border-2 border-background flex items-center justify-center text-[8px] font-bold">
                              {p[0]}
                            </div>
                          ))}
                        </div>
          ...
          <TabsContent value="leagues" className="space-y-6 pt-6">
          ...
          <TabsContent value="leaderboard" className="pt-6">
            <div className="glass-panel rounded-[2rem] overflow-hidden border-white/5">
              ...
              <div className="divide-y divide-white/5">
                {[
                  { name: 'Tiger Woods', hcp: '+4.2', rounds: 890, rank: 1 },
                  { name: 'Rory McIlroy', hcp: '+3.1', rounds: 412, rank: 2 },
                  { name: 'Jon Rahm', hcp: '+2.8', rounds: 356, rank: 3 },
                  { name: 'You', hcp: '4.5', rounds: 24, rank: 142, isSelf: true },
                ].map((entry) => (
                  <div key={entry.rank} className={`p-4 flex items-center gap-4 ${entry.isSelf ? 'bg-primary/10' : ''}`}>
